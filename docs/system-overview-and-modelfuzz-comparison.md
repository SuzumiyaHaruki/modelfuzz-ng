...
NG 会持久化 HardState 和 ConfState，并且只在创建 snapshot 后压缩日志；扩展 TLA+ profile 与 Concrete Oracle 共同检查 snapshot 边界、安装、拒绝、传输状态和 committed prefix，减少由不完整应用契约制造假状态的可能。

#### 5.2.4 本地 panic 的因果机制

本地出现的空 snapshot panic 都依赖 n/3+1 mutant，调用链为 `stepLeader → maybeSendAppend → maybeSendSnapshot → panic("need non-empty snapshot")`：

| 顺序 | 机制 |
| ---: | --- |
| 1 | 弱 quorum 让缺少已提交日志的节点成为 Leader，首先破坏 Leader Completeness |
| 2 | 日志更完整的 Follower 返回超过该 Leader 本地日志的确认位置，使 Leader Progress 越过自身 last index |
| 3 | Leader 无法继续构造 `MsgApp`，转入 snapshot 路径，但应用只有空初始 snapshot，最终触发 panic |

因此，这些 panic 是 seeded quorum mutant 的下游故障信号，不是独立的第二个反例。
