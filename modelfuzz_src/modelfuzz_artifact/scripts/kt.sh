...
if [[ "$benchmark" == "redis" || "$benchmark" == "etcd" ]]; then
    tlc_run.sh Raft RAFT_3_3 -mapperparams "name=raft;abstract" > /tmp/tlc.log 2>&1 &
    tlc_pid=$!
...
echo "Running test script for $benchmark"
