...
B2-v1 把同一父轨迹的所有 child 都绑定到最早的新状态，没有实现“每个新状态分别作为
搜索前沿”的原始意图。B2-v2 修改为：

- Fuzzer 显式遍历 `Guidance.NewStates`，每个状态分别生成 `MutPerTrace` 个 child；
- located 状态的每个 child 只冻结到该状态自己的 `Origin.Step`；initial state 仍按原始
  全局 Mutator 生成同等数量的 child；
- child 在队列中携带目标 state key 和 step，执行后直接用本轮 TLC transition 检查目标
  状态是否保持；
- 后缀算子交换次数不超过原始上限，并按剩余候选缩减；某类算子没有足够候选时只跳过
...
结果说明B2-v1前缀保持确实减少无效/空执行并提高模型路径多样性，但没有提高新模型状态
覆盖；该版本更偏向围绕最早状态 exploit。B2-v1保留为独立基线，不据此进入AI阶段。

B2-v2完成逐状态目标绑定、自适应后缀算子和在线目标保持统计后，首次使用同一配置复验：

| 指标 | global | per-state prefix | 变化 |
|---|---:|---:|---:|
| 最终状态均值 | 51.8 | 38.6 | -25.5% |
| coverage AUC 均值 | 2843.6 | 2419.0 | -14.9% |
| unique state traces 总数 | 146 | 145 | -0.7% |
| 总运行时间 | 25.52s | 26.02s | +2.0% |
| 空 state trace 数 | 282 | 120 | -57.4% |

B2-v2 的每个新状态都获得3次独立尝试：579次尝试生成558个child，拒绝21次（3.6%）；
实际执行360条mutation和40条random。首次运行时，345条带目标的child中有7条被报告为
未重现。后续重复实验确认这些丢失不是后缀越界或TLA+动作多后继：
...
在同一个修复版TLC进程上重跑逐状态Prefix，覆盖曲线也与修复前逐iteration完全一致，说明
TLC问题影响的是归因与目标保持测量，不是此前的搜索结果：

| 指标 | fixed global | fixed per-state prefix | 变化 |
|---|---:|---:|---:|
| 最终状态均值 | 51.8 | 38.6 | -25.5% |
| coverage AUC均值 | 2843.6 | 2419.0 | -14.9% |
| unique state traces总数 | 146 | 145 | -0.7% |
| 总运行时间 | 26.43s | 27.13s | +2.7% |
| 空state trace数 | 282 | 120 | -57.4% |

本次579次尝试仍生成558个child并拒绝21次；实际执行360条mutation、40条random。
345条带目标child全部重现了归因点对应的原始transition post-state，目标保持率为100%；
此前7次“目标丢失”全部由TLC保存可变历史状态造成。带目标child共发现95个全局新模型状态，
其中58条child至少发现一个新状态，interesting率为16.8%。按目标深度分组：

| 目标step | 执行数 | 保持率 | 新状态数/执行 | interesting child率 |
|---|---:|---:|---:|---:|
| 0–24 | 101 | 100% | 45/101（44.6%） | 26/101（25.7%） |
| 25–49 | 103 | 100% | 35/103（34.0%） | 22/103（21.4%） |
| 50–74 | 54 | 100% | 10/54（18.5%） | 6/54（11.1%） |
| 75+ | 87 | 100% | 5/87（5.7%） | 4/87（4.6%） |

按归因点的模型动作分组，`ElectLeader`的interesting率为31.0%（26/84），
`ClientRequest`为24.7%（20/81），`HandleAppendEntriesRequest`为10.0%（10/100），
`HandleAppendEntriesResponse`仅为2.5%（2/80）。对应的新状态/执行分别为60.7%、33.3%、
14.0%和3.8%。这不是动作本身的独立因果效应，但足以表明“每个新状态固定分配相同energy”
不合理：深层状态和普通AppendEntries响应之后的随机后缀大量消耗预算，却很少进入新区域。

排除前20条seed replay后，fixed global有78/400条interesting执行并发现186个新状态，
fixed Prefix有64/400条interesting执行并发现120个新状态；与此同时空state trace从
205/400降到43/400。Prefix因此确实避免了大量完全无模型轨迹的执行，但没有把节省的预算
转化成更高覆盖。

B2-v2仍未提高状态覆盖，而且比B2-v1更差；问题不在前缀未保持，而在通用随机后缀没有利用
状态迁移语义，且对所有深度和动作平均分配energy。在重新定义不同目标的energy、设计关键
行为关系变异前，不继续调global/prefix比例，也不进入AI阶段。
