...
// VisitGraph 记录 TLC 状态之间的访问转移图，用于后续可视化分析。
//
// Guider 关心覆盖了多少唯一状态；VisitGraph 进一步记录这些状态之间如何被 trace 连接起来。
// 这有助于分析搜索是不是卡在局部区域，或者某些 guider 是否能进入新的模型区域。
...
func (v *VisitGraph) Update(trace []State) {
	// 将一次 TLC 返回的状态序列合并进全局访问图。
	// 每条边表示一次模型状态转移，Visits 记录该状态作为路径节点被经过的次数。
...
func (v *VisitGraph) record(recordPath string, key string) {
	// 每个 guider/benchmark 可以输出一份 visit graph。
...
type VisitGraphNode struct {
	// Key 是 TLC 返回的状态 hash，State 是可读状态表示。
	Key    int64
	State  string
	Visits int
